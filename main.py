import random
from upanddown import up_and_down_game
...
if __name__ == "__main__":
    if not start_game(): 
        ...
        while True:
            ...
            elif selected_game == "UP&DOWN 게임": # ==========내가 건드린 부분 시작===========
                winner = up_and_down_game(player_name, [player_name] + list(opponents_with_limits.keys()), starting_player)
                losers = [name for name in player_lives.keys() if name != winner]
                for loser in losers:
                    player_lives[loser] -= 1
                    print(f"{loser}님의 남은 주량: {player_lives[loser]}잔")
                print(f"{winner}님의 남은 주량: {player_lives[winner]}잔")
